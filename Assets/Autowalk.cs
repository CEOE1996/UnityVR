...
public class Autowalk : MonoBehaviour
{
    ...
    [Tooltip("Max Duplicate Number Speed on Click")]
    public int MaxSpeed;
    ...
    void Update()
    {
        ...
        if (walkWhenTriggered && !walkWhenLookDown && isWalking < MaxSpeed && Input.GetButtonDown("Fire1"))
        {
            isWalking++;
        }
        else if (walkWhenTriggered && !walkWhenLookDown && isWalking == MaxSpeed && Input.GetButtonDown("Fire1"))
        {
            isWalking = 0;
        }

        // Walk when player looks below the threshold angle 
        if (walkWhenLookDown && !walkWhenTriggered && isWalking < MaxSpeed &&
            mainCamera.transform.eulerAngles.x >= thresholdAngle &&
            mainCamera.transform.eulerAngles.x <= RIGHT_ANGLE)
        {
            isWalking++;
        }
        else if (walkWhenLookDown && !walkWhenTriggered && isWalking == MaxSpeed &&
                 (mainCamera.transform.eulerAngles.x <= thresholdAngle ||
                 mainCamera.transform.eulerAngles.x >= RIGHT_ANGLE))
        {
            isWalking = 0;
        ...
        if (isWalking > 0)
        {
            Vector3 direction = new Vector3(mainCamera.transform.forward.x, 0, mainCamera.transform.forward.z).normalized * (speed * isWalking)* Time.deltaTime;
